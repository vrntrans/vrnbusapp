...
class StationInfoDialog {
    companion object {
                    ...
                    try {
                        if (dialog == null || !dialog.isShowing || DataManager.activeStationId != stationOnMap.id) {
                            ...
                            if (stationInfoResult.status == RequestStatus.Ok && stationInfoResult.data != null) {
                                ...
                                if (DataManager.activeStationId == mStation.id) DataManager.activeStationId = 0
                                Toast.makeText(
                                    activity,
        ...
        private fun updateWithStationInfo(station: StationObject, stationInfo: BusesOnStationObject) {
            ...
            if (favorites != null) {
                val favRoutes =
                    routesForList.filter { favorites.contains(it.bus.routeName) && it.arrivalTime != null }
                val noFavRoutes =
                    routesForList.filterNot { favorites.contains(it.bus.routeName) && it.arrivalTime != null }
